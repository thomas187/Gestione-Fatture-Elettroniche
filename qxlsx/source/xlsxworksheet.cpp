...
void WorksheetPrivate::saveXmlCellData(QXmlStreamWriter &writer, int row, int col, QSharedPointer<Cell> cell) const
{
    ...
    else if (/* modifica per non danneggiare le formule */ (cell->cellType() == Cell::ErrorType && cell->hasFormula()) || /* fine aggiunta */cell->cellType() == Cell::NumberType) // 'n'
    {
