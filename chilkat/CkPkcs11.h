...
class CK_VISIBLE_PUBLIC CkPkcs11  : public CkMultiByteBase
{
	...
	// smart cards do not need this. Don't set this property unless you know for sure
	// your smart card needs it.
	const char *sigContextPin(void);
	// If the smart card or token requires a signature context login within the signing
	// operation, then set this property to the context-specific signature PIN. Most
	// smart cards do not need this. Don't set this property unless you know for sure
	// your smart card needs it.
	void put_SigContextPin(const char *newVal);
	...
	// information.
	unsigned long CreatePkcs11Object(CkJsonObject &attrs);
	...
	// 
	bool ExportPublicKey(unsigned long keyHandle, CkPublicKey &pubKey);
	...
	// OpenSession.
	// 
	bool FindAllKeys(const char *keyClass, CkJsonObject &json);
	...
	unsigned long FindObject(CkJsonObject &jsonTemplate);
	...
	// See the example linked below for more details.
	// 
	bool GenEcKey(CkJsonObject &publicAttrs, CkJsonObject &privateAttrs, CkJsonObject &jsonOut, CkPublicKey &pubKey);
	...
	// See the example linked below for more details.
	// 
	bool GenRsaKey(CkJsonObject &publicAttrs, CkJsonObject &privateAttrs, CkJsonObject &jsonOut, CkPublicKey &pubKey);
	...
	// failure.
	// 
	unsigned long GenSecretKey(const char *keyType, CkJsonObject &jsonTemplate);
	...
	unsigned long ImportPrivateKey(CkPrivateKey &privKey, CkJsonObject &jsonTemplate);


	// Imports an SSH private key onto the HSM. Returns the handle of the unwrapped
	// key, or 0 if failed.
	unsigned long ImportSshKey(CkSshKey &sshKey, CkJsonObject &jsonTemplate);
	...
	// 
	bool QuickSession(int userType, const char *pin);
