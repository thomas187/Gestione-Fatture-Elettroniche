...
class CK_VISIBLE_PUBLIC CkHttpW  : public CkClassWithCallbacksW
{
	...
	// property should be set to a JSON string containing the information needed to
	// compute and automatically add the HTTP Signature. See the example(s) below.
	const wchar_t *authSignature(void);
	// If set, then automatically adds the Authorization: Signature keyId="...",
	// algorithm="...", headers="...", signature="..." header to all requests. This
	// property should be set to a JSON string containing the information needed to
	// compute and automatically add the HTTP Signature. See the example(s) below.
	void put_AuthSignature(const wchar_t *newVal);
	...
	// This is only used if a set of temporary security credentials were obtained by
	// calling the AssumeRole action of the AWS Security Token Service.
	const wchar_t *awsSessionToken(void);
	// This is only used if a set of temporary security credentials were obtained by
	// calling the AssumeRole action of the AWS Security Token Service.
	void put_AwsSessionToken(const wchar_t *newVal);
	...
	// Authorization header. The oauth_body_hash contains the SHA-256 hash of the HTTP
	...
	// 
	bool CreateTimestampRequest(const wchar_t *hashAlg, const wchar_t *hashVal, const wchar_t *reqPolicyOid, bool addNonce, bool reqTsaCert, CkBinDataW &timestampToken);
	...
	// "https://www.chilkatsoft.com/default.asp" is the URL passed in, then
	...
	// The caller is responsible for deleting the object returned by this method.
	CkHttpResponseW *QuickRequestParams(const wchar_t *verb, const wchar_t *url, CkJsonObjectW &json);

	// Creates an asynchronous task to call the QuickRequestParams method with the
	// arguments provided. (Async methods are available starting in Chilkat v9.5.0.52.)
	// The caller is responsible for deleting the object returned by this method.
	CkTaskW *QuickRequestParamsAsync(const wchar_t *verb, const wchar_t *url, CkJsonObjectW &json);
	...
	bool SetAuthPrivateKey(const wchar_t *publicKeyId, CkPrivateKeyW &privKey);

	// Sets the AuthToken property. The sb contains the OAuth2 access token to be
	// used.
	bool SetAuthTokenSb(CkStringBuilderW &sb);
