// CkSshKeyW.h: interface for the CkSshKeyW class.
//
//////////////////////////////////////////////////////////////////////
...
// CLASS: CkSshKeyW
class CK_VISIBLE_PUBLIC CkSshKeyW  : public CkWideCharBase
{
	...
	CkSshKeyW &operator=(const CkSshKeyW &);

    public:
	CkSshKeyW(void);
	virtual ~CkSshKeyW(void);

	

	static CkSshKeyW *createNew(void);
	...
	// The password to be used when importing or exporting encrypted private keys. If
	// the private key is in an unencrypted format, the password is ignored.
	...
	// -----END RSA PRIVATE KEY-----
	bool FromOpenSshPrivateKey(const wchar_t *keyStr);
	...
	bool FromOpenSshPublicKey(const wchar_t *keyStr);
	...
	bool FromPuttyPrivateKey(const wchar_t *keyStr);
	...
	// zg==
	// ---- END SSH2 PUBLIC KEY ----
	bool FromRfc4716PublicKey(const wchar_t *keyStr);
	...
	// 
	bool GenerateEcdsaKey(const wchar_t *curveName);
	...
	// Convenience method for loading an entire text file into an in-memory string.
	const wchar_t *loadText(const wchar_t *filename);

	// Convenience method for saving the contents of a string variable to a file.
	bool SaveText(const wchar_t *strToSave, const wchar_t *filename);
