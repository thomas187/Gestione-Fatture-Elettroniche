...
class CK_VISIBLE_PUBLIC CkPdfW  : public CkClassWithCallbacksW
{
	...
	// PDF.
	bool GetUnsignedSigFields(CkJsonObjectW &json);
	...
	//     to sign an existing unsigned signature field. Chilkat will automatically scale
	//     the visual appearance (text + graphics) to fit the pre-existing signature field.
	//     When fillUnsignedSignatureField is specified, it is not necessary to set
	//     appearance.x, appearance.y, appearance.fontScale, etc. Starting in v9.5.0.90,
	//     the unsignedSignatureField option (see below) can be used to specify the
	//     unsigned signature field to be used. Otherwise, the 1st available unsigned field
	//     is used.
	...
	//     info.* - (added in Chilkat v9.5.0.90) Provides the ability to add or update
	//     the PDF document's "/Info", such as "Creator", "Producer", "Title", etc. Any
	//     number of arbitrary member names can be specified and these will be added to the
	//     document's /Info dictionary. However, Chilkat will ignore "ModDate" and
	...
	//     unsignedSignatureField - (added in Chilkat v9.5.0.90) Can be set to specify
