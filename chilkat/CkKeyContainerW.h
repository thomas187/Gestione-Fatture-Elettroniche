...
class CK_VISIBLE_PUBLIC CkKeyContainerW  : public CkWideCharBase
{
	...
	bool DeleteKey(const wchar_t *keyName, const wchar_t *storageProvider);
	...
	bool ExportKey(const wchar_t *keyName, const wchar_t *storageProvider, bool silentFlag, CkPrivateKeyW &key);
	...
	bool GetKeys(const wchar_t *storageProvider, CkJsonObjectW &json);
	...
	// }
	// 
	bool GetStorageProviders(CkJsonObjectW &json);
	...
	bool ImportKey(const wchar_t *keyName, const wchar_t *storageProvider, bool allowOverwrite, bool allowExport, CkPrivateKeyW &key);
